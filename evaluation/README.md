...
Для обычного запуска в репозитории лежат компактные XML-fixtures с 18 размеченными медицинскими фрагментами, подготовленными по закреплённым на конкретном commit материалам MedQuAD/NIDDK. Поэтому подготовка 60 вопросов не требует подключения к GitHub. Для исходных вопросов создаются шаблонные переформулировки; у каждой записи сохраняется соответствующий `gold_chunk_ids`. Поле `provenance` показывает, является вопрос исходным или шаблонной переформулировкой.

MedQuAD распространяется по лицензии CC BY 4.0. Исходные URL, commit, путь fixture и отметка о режиме подготовки сохраняются в `evaluation/data/sources.json`.

Полные закреплённые MedQuAD XML можно заново скачать командой:

```bash
python -m evaluation.prepare_dataset --questions 60 --refresh
```

В режиме `--refresh` загрузчик делает несколько попыток с экспоненциальной задержкой и переключается с `raw.githubusercontent.com` на GitHub Contents API, возвращающий raw-содержимое. Стандартный benchmark не использует `--refresh`.
...
Подготовка тестового набора теперь выполняется из локальных fixtures. Первый запуск по-прежнему скачивает веса моделей в Docker volume `benchmark_huggingface_cache`. Результаты сохраняются на хосте в `evaluation/results/`.

После обновления ветки Docker должен переиспользовать уже собранный слой с Python-зависимостями. Для принудительной пересборки только изменённого кода достаточно повторить ту же команду; `pip install` обычно останется в build cache.
...
Набор небольшой, англоязычный и содержит шаблонные переформулировки. Bundled fixtures являются компактной производной от исходных MedQuAD/NIDDK материалов, а не полными копиями исходных страниц. Один fixture `Answer` используется как один gold-фрагмент. Набор подходит для контролируемого инженерного сравнения retriever-конфигураций, но не является клинической валидацией качества медицинских ответов. `citation_accuracy` здесь — строгая проверка попадания top-1 цитаты в gold-разметку, а не экспертная оценка полного сгенерированного ответа.
