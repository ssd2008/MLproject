...
5. находит фрагменты по смысловой близости;
6. при явно включённой экспериментальной опции повторно ранжирует кандидатов cross-encoder-моделью;
...
`reranking` — повторное ранжирование найденных кандидатов отдельной моделью. В Асси reranking сохранён для экспериментов, но не используется по умолчанию.
...
Модель преобразует вопрос и чанки в нормализованные векторы размерности 1024. Qdrant выполняет dense semantic search и возвращает ближайшие фрагменты по косинусной близости.

Dense retrieval выбран на основании воспроизводимого эксперимента на 3 медицинских материалах, 18 gold-фрагментах и 60 вопросах:
...
Dense дал лучший MRR и лучший первый результат при приемлемой интерактивной задержке. Полная методология и ограничения описаны в [retrieval benchmark](../evaluation/README.md), а интерпретация — в [результатах эксперимента](../evaluation/RESULTS.md).
...
Cross-encoder получает вопрос и текст кандидата одновременно и оценивает их совместную релевантность. В проведённом эксперименте reranker увеличил Recall@5 с `0.983` до `1.000`, но снизил MRR с `0.865` до `0.715`, снизил Top-1 gold accuracy с `78.3%` до `58.3%` и увеличил медианную задержку примерно в 18.6 раза.

Поэтому reranker отключён по умолчанию и используется только как экспериментальная опция.
...
Для конкретного запроса дополнительно передаётся:
...
При `RERANKER_ENABLED=false` приложение всегда использует Dense retrieval, а `rerank_score` в ответе равен `null`.

```text
normalized_retrieval_score = clamp((retrieval_score + 1) / 2, 0, 1)

final_score =
    normalized_retrieval_score,                         Dense по умолчанию
    0.25 * normalized_retrieval_score
      + 0.75 * rerank_score,                            experimental reranker
```
  ...
  -> Nginx / React frontend
      -> FastAPI routers
          -> services
              ...
              -> embedding / optional reranker / answer providers
              -> Qdrant repository
```
...
`GET /api/v1/health` проверяет подключение к PostgreSQL и Qdrant. Для embedding, ASR и answer возвращается настроенная реализация. Reranker по умолчанию имеет статус `disabled`; при `RERANKER_ENABLED=true` возвращается его настроенный backend. Healthcheck не загружает модели и не выполняет тестовый inference.
...
При стандартном запуске `model-init` скачивает embedding- и Whisper-модели, но пропускает reranker. После установки `RERANKER_ENABLED=true` reranker также предварительно загружается в Hugging Face cache.
...
- PDF до 10 МБ читается в память перед извлечением текста; видео сохраняется потоково блоками;
...
- retrieval benchmark небольшой и не заменяет production-scale evaluation dataset;
