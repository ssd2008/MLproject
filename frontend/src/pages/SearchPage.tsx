...
import { formatScore, pageLabel, SOURCE_LABELS } from "../utils";
    ...
    <div className="page-stack">
      <section className="page-heading">
        <div>
          ...
          <p>Проверьте, какие фрагменты retriever и reranker считают наиболее релевантными.</p>
        </div>
      </section>
      ...
      <div className="split-layout split-layout--search">
          ...
          <form onSubmit={(event) => void handleSubmit(event)}>
            <label className="field">
              ...
              <textarea
                ...
                placeholder="Например: Какие признаки характерны для артериальной гипертензии?"
              />
            </label>
              ...
              <div className="advanced-fields">
                <div className="form-grid form-grid--two">
                  <label className="field">
                    ...
                    <input value={specialty} onChange={(event) => setSpecialty(event.target.value)} placeholder="cardiology" />
                  </label>
                  <label className="field">
                    <span>Язык</span>
                    <input value={language} onChange={(event) => setLanguage(event.target.value)} placeholder="ru" />
                  </label>
                </div>
                <label className="field">
                ...
                <div className="form-grid form-grid--two">
                  <label className="field">
                    <span>Результатов top_k</span>
                    <input type="number" min={1} max={100} value={topK} onChange={(event) => setTopK(Number(event.target.value))} />
                  </label>
                  <label className="field">
                    <span>Кандидатов candidate_k</span>
                    <input type="number" min={1} max={300} value={candidateK} onChange={(event) => setCandidateK(Number(event.target.value))} />
                  </label>
                </div>
            ...
            <Button className="full-width" type="submit" loading={loading} disabled={!query.trim()} icon={<span>⌕</span>}>
              Найти фрагменты
            </Button>
          </form>
        </aside>
        ...
        <section className="search-results">
          ...
          {loading ? (
            <div className="panel center-loader center-loader--large">
              <Spinner />
              <p>Ищем релевантные фрагменты...</p>
            </div>
          ) : !response ? (
            <div className="panel">
              <EmptyState icon="⌕" title="Введите вопрос для поиска" text="Здесь появятся chunks, scores и ссылки на исходные материалы." />
            </div>
          ) : response.results.length === 0 ? (
            <div className="panel">
              <EmptyState icon="∅" title="Результатов нет" text="Попробуйте изменить запрос или снять часть фильтров." />
            </div>
          ) : (
            <>
                      ...
                      <div className="result-card__header">
                        <div>
                          ...
                          <div className="tag-row">
                            ...
                            <Tag>{pageLabel(result.page_start, result.page_end)}</Tag>
                          </div>
                        </div>
